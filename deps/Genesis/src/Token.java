...
public class Token {
    ...
    class PositionInfo { // For tracking the position in scanning
       ...
       int charPos () { return charPos;}
    }
    static Stack<PositionInfo> positionInfoStack = new Stack<PositionInfo>();
   ...
   // static int lineNo = 1;
    ...
    public void processInput()   {
         if (debug) System.out.println("In Token constructor (String filename) ");
        val = ""; 
        ch = ' ';
    ...
    void mark() {
      ...
      markval = val;
      if (debug) System.out.println("Marking input stream");
      if (debug) System.out.println("  ch == '" + ch + "'");
      if (debug) System.out.println("  lineNo == " + sourcePgm.lineNo());
      if (debug) System.out.println("  charPos == " + sourcePgm.charPos());
      if (debug) System.out.println("  markPos == " + markpos);
    }
    ...
    void reset() {
      ...
      val = markval; 
      if (debug) System.out.println("Reseting input stream:");
      if (debug) System.out.println("  ch == '" + ch + "'");
      if (debug) System.out.println("  lineNo == " + sourcePgm.lineNo());
      if (debug) System.out.println("  charPos == " + sourcePgm.charPos());
    }
    ...
    void gnc() {   
            ...
            if (bufferPos >= sourcePgm.length()) {
               if (debug) System.out.println ("End of source pgm encountered at"
            ...
            if (debug) System.out.println("Token.gnc:: Entering: ch = '"+ ch+"'" + " bufferPos="+bufferPos);
            if (debug) System.out.println("Token.gnc:: " + sourcePgm.line());
            if ( c == -1)  {
               ch = (char)26; // ^Z  ljm
               eof = true;
            }
        ...
        if (debug) System.out.println("Token.gnc:: ch = '"+ ch+"'" 
                    + " newline="+newline
                    + " eof="+eof
              );
        if (debug) System.out.println("Token.gnc: sourcePgm='" + sourcePgm +"'");
        if (debug) System.out.println("Token.gnc: sourcePgm.pos='" + sourcePgm.charPos() +"'");
    }
    ...
    public void get () {  
        ...
        semiBreak = false;
        lineBreak = false;
        if (debug) System.out.println("Entering get");
        while (more() && (ch == ' ' || ch == '\n' || ch == '\t' || ch == ';' || ch == ',')) {
            if (ch == '\n')  {
               lineBreak = true;
            }
        ...
        if ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') 
            ...
            if (val.equals("while"))              tokenType = Parser.whileSym;
            else if (val.equals("let"))           tokenType = Parser.letSym;
            else if (val.equals("alias"))         tokenType = Parser.aliasSym;
            else if (val.equals("generate"))      tokenType = Parser.generateSym;
            else if (val.equals("task"))          tokenType = Parser.taskSym;
            else if (val.equals("function"))      tokenType = Parser.functionSym;
            else if (val.equals("procedure"))     tokenType = Parser.procedureSym;
            else if (val.equals("print"))         tokenType = Parser.printSym;
            else if (val.equals("name"))          tokenType = Parser.nameSym;
            else if (val.equals("emit"))          tokenType = Parser.emitSym;
            else if (val.equals("select"))        tokenType = Parser.selectSym;
            else if (val.equals("to"))            tokenType = Parser.toSym;
            else if (val.equals("through"))       tokenType = Parser.toSym;
            else if (val.equals("thru"))          tokenType = Parser.toSym;
            else if (val.equals("downto"))        tokenType = Parser.downtoSym;
            else if (val.equals("file"))          tokenType = Parser.fileSym;
            else if (val.equals("from"))          tokenType = Parser.fromSym;
            else if (val.equals("by"))            tokenType = Parser.bySym;
            else if (val.equals("call"))          tokenType = Parser.callSym;
            else if (val.equals("return"))        tokenType = Parser.returnSym;
            else if (val.equals("of")) {
                 if (prevTokenType == Parser.idSym ) get(); // otherwise leave it
            }
            else if (val.equals("while"))         tokenType = Parser.whileSym;
            else if (val.equals("@f"))            tokenType = Parser.atbeginSym;
            else if (val.equals("@first"))        tokenType = Parser.atbeginSym;
            else if (val.equals("@l"))            tokenType = Parser.atendSym;
            else if (val.equals("@last"))         tokenType = Parser.atendSym;
            else if (val.equals("@i"))            tokenType = Parser.ateachSym;
            else if (val.equals("@iter"))         tokenType = Parser.ateachSym;
            else if (val.equals("@each"))         tokenType = Parser.ateachSym;
            else if (val.equals("@beginning"))    tokenType = Parser.atbeginSym;
            else if (val.equals("@begin"))        tokenType = Parser.atbeginSym;
            else if (val.equals("@ending"))       tokenType = Parser.atendSym;
            else if (val.equals("@end"))          tokenType = Parser.atendSym;
            else if (val.equals("@each"))         tokenType = Parser.ateachSym;
            else if (val.equals("and"))           tokenType = Parser.andSym;
            else if (val.equals("or"))            tokenType = Parser.orSym;
            else if (val.equals("not"))           tokenType = Parser.notSym;
            else if (val.equals("true"))          {tokenType=Parser.trueSym;}
            else if (val.equals("false"))         {tokenType = Parser.falseSym;}
            else if (val.equals("otherwise"))     tokenType = Parser.otherwiseSym;
            else if (val.equals("insert"))        tokenType = Parser.insertSym; 
            else if (val.equals("delete"))        tokenType = Parser.deleteSym; 
            else if (val.equals("after"))         tokenType = Parser.afterSym; 
            else if (val.equals("before"))        tokenType = Parser.beforeSym; 
            else if (val.equals("in"))            tokenType = Parser.inSym; 
	    else if (val.equals("append"))        tokenType = Parser.appendSym; 
            else if (val.equals("onto"))          tokenType = Parser.toSym; 
            else if (val.equals("stop"))          tokenType = Parser.stopSym; 
            else if (val.equals("echo"))          tokenType = Parser.echoSym; 
            else if (val.equals("unalias"))       tokenType = Parser.unaliasSym; 
            else if (val.equals("generator"))     tokenType = Parser.generatorSym;
            else if (val.equals("when"))     tokenType = Parser.whenSym;
            else if (val.equals("until"))     tokenType = Parser.untilSym;
            else if (val.equals("iter"))     tokenType = Parser.iteratorSym;
            else if (val.equals("prepend"))        tokenType = Parser.prependSym; 
/*
...
*/
		...
		gnc();
            ...
            tokenType = Parser.numberSym;
             ...
             val += ch; gnc();
             if (val.equals("("))          tokenType = Parser.lparenSym;
             else if (val.equals(")"))     tokenType = Parser.rparenSym;
             else if (val.equals("*"))     tokenType = Parser.starSym;
             else if (val.equals("%"))     tokenType = Parser.percentSym;  // chad added for %
             else if (val.equals("+"))     tokenType = Parser.plusSym;
             else if (val.equals("{")) {   
             ...
             else if (val.equals("}"))     tokenType = Parser.rbraceSym;
             else if (val.equals(";"))     tokenType = Parser.semiSym;
             else if (val.equals(":"))     tokenType = Parser.colonSym;
             else if (val.equals("|"))     tokenType = Parser.pipeSym;
             else if (val.equals(","))     tokenType = Parser.commaSym;
             else if (val.equals("[")) {   
             ...
             else if (val.equals("]"))     tokenType = Parser.rbracketSym;
             else if (val.equals("="))     tokenType = Parser.eqSym;
             else if (val.equals("&"))     tokenType = Parser.andSym;
             else if(val.equals("/")) {
                 if (ch == '/') {
                    while (ch != '\n'){ gnc();  }
                    tempCounter=1;
...
*/
        ...
        // debug=false;
        if (debug) System.out.println("Token.get::val = '" + val 
                                                      + "' " 
                        + "type = " 
                        + tokenType 
                        + " ch='" 
   ...
   public String toString () {
       String s;
       if (debug) System.out.println("Looking up " + tokenType );
        ...
        while (line != null) {
          boolean incStmt=false;
            ...
            if (!incStmt) {
                ...
                if (debug) System.out.println("sourcePgm:'" + sourcePgm +"'");
            }
            line = io.readLine();
        }
    }
    ...
    public static void main (String [] args)  {
       GenesisIO io = new GenesisIO();
       io.setInputFile( args[0] );
       Token t = new Token (args[0]);
       t.setSource(args[0]);
       t.appendInput(args[0]);
       t.processInput();
       System.out.println(t.sourcePgm);
       // debug = false;
       Parser.init();
       /*
       for (int i = 0 ; i < 55; i++ ) {
         System.out.println (Parser.tokenStr[i]);
       }
*/
       while (t.more()) {
           t.get();
