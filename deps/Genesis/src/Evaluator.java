...
public class Evaluator
{
  ...
  boolean testOp( Node tn, GenesisVal v ) { return tn.info.val.eq( v ); }
  ...
  { 
    // for tests like
    //                 if ( isIterator( "x"))
    // System.out.println("In isIterator checking " + s);
    StickyNote sn = scope.find(s);
    if (sn == null)   // if it is not in the scope
       return false; 
    else {           //  see if it's value is an Iterator
       //System.out.println("Returning " + (sn.val instanceof GenesisList));
  ...
  public static void printError(String msg, TreeNode tn) {
     ...
     while (tn != null && tn.lineNo() == 0 ) tn =  (TreeNode)tn.left;
  ...
  String getLabel (Node tn) {
     // System.out.println ("Entering getLabel with " + tn + tn.getLabel());
     //String ans = evalExpression (new TreeNode(tn.getLabel())).toString();
     String ans = "Fix me";
     return ans;
  ...
  TreeNode findContext (TreeNode context, TreeNode tn, Node key) {
     TreeNode result;
     // System.out.println("Entering findcontext with:");
     // System.out.println(context);
     // System.out.println(tn);
     // System.out.println(key);
     // System.out.println("-----------------------------");
     if (tn == null) result =  null;
     else if (tn == key) result = context;
     else { 
  ...
  StickyNote evalStmt( Node tn )
  {
    ...
    if (autotrace && ! testOp ( tn, OpVal.stmtListOp)) {
          System.out.print("==> "); // Executing instruction:"+ ((TreeNode) tn).prettyNo());
          System.out.print(tn);  // will invoke toString, a pretty printer
    }
       ...
       if (stopAt.contains(new Integer(((TreeNode) tn).prettyNo()))
          ...
          while  (doAgain) {
             ...
             try {
                ...
                if (x >= 0) {
                   char ch = (char) x;
                   if (ch == 'n')  {doAgain = false;}
                   else if (ch == 'L') {
                      System.out.println(root);  // will invoke toString, a pretty printer
                   }
                   else if (ch == 'l') {
                      TreeNode context = findContext(tn);
                      System.out.println(context);  // will invoke toString, a pretty printer
                   }
                   else if (ch == '.') {
                      System.out.println(tn);  // will invoke toString, a pretty printer
                   }
    ...
    if      ( testOp( tn, OpVal.idNameOp ) )return  evalIdName( tn );
    else if ( testOp( tn, OpVal.idAliasOp ) )return  evalIdAlias( tn );
    else if ( testOp( tn, OpVal.printOp ) ) return evalPrint( tn );
    else if ( testOp( tn, OpVal.selectOp ) ) return evalSelect( tn );
    else if ( testOp( tn, OpVal.stmtListOp ) ) return evalStmtList( tn );
    else if ( testOp( tn, OpVal.functionDefOp ) )return  evalFunctionDef( tn );
    else if ( testOp( tn, OpVal.procedureDefOp ) )return  evalFunctionDef( tn );
    else if ( testOp( tn, OpVal.generatorDefOp ) )return  evalFunctionDef( tn );
    else if ( testOp( tn, OpVal.returnStmtOp ) ) return evalExpression( tn.left() );
    else if ( testOp( tn, OpVal.procedureCallOp ) ) {evalFunctionCall( tn ); }
    else if ( testOp( tn, OpVal.whileOp ) ) evalWhileStmt( tn );
    else if ( testOp( tn, OpVal.pipeOp ) ) evalPipe( tn );
    else if ( testOp( tn, OpVal.deleteOp ) ) evalDelete( tn );
    else if ( testOp( tn, OpVal.appendOp ) ) evalAppend( tn );
    else if ( testOp( tn, OpVal.prependOp ) ) evalPrepend( tn );
    else if ( testOp( tn, OpVal.insertAfterOp ) ) evalInsertAfter( tn );
    else if ( testOp( tn, OpVal.insertBeforeOp ) ) evalInsertBefore( tn );
    else if ( testOp( tn, OpVal.stopStmtOp ) ) evalStop( tn );
    else if ( testOp( tn, OpVal.echoStmtOp ) ) evalEcho( tn, false );
    //else if ( testOp( tn, OpVal.generateStmtOp ) ) evalGenerate( tn ); // standalone
    else if ( testOp( tn, OpVal.unaliasStmtOp ) ) evalUnalias( tn ); // standalone
    ...
    else if ( testOp( tn, OpVal.generatorCallOp ) ) evalFunctionCall( tn );
    else if ( testOp( tn, OpVal.nullStmtOp ) ) /* Do nothing */;
    else
    {
      ...
      if (isIterator(id)) { 
       //System.out.println("letIdNameSN: Let " + id + " name " + sn);
        ...
        if ( gl.empty() || gl.off() ) { // adding the first node to the list
         ...
         // debug= true;
         if (debug) System.out.println ("Assigning " + id + "-to-"  + sn  + " of type " + sn.val.getClass()  );
         scope.setName( id, sn );
         // debug = false;
...
*/
  ...
  String addr(Object t) {return t.toString();}
  ...
  { GenesisList result = null;
    ...
    if ( tn.info.val instanceof StringVal )
    {
      ...
      if (n == null)   //  either a null iterator or a name that was never defined
      {
        //printError("Label '" + id + "' references no value", tn);
         ...
         if (debug) System.out.println("Value/class of expression is "+ n.val + "" + n.val.getClass() );
         ...
         if (n.val instanceof GenesisList) {
            ...
            if (debug) System.out.println ("n.val is a GenesisList" + n.val);
         }
            ...
            while (result.on()) result.move(); // to indicate it's a top-level list
         }
   ...
   // debug = true;
    if (debug) { System.out.println("evalRef returns " + result + "/" + result.getClass() ); }
    return result;
  }
  ...
  StickyNote evalExp( Node tn )  // Same as evalExpression, but doesn't evaluate
  {
    ...
    if ( tn.info.val instanceof StringVal )
    {
      ...
      if (debug) System.out.println("evalExp returning:" + n.toString()); 
      // return (n);
      result = n;
    ...
    else if ( testOp( tn, OpVal.addOp ) ) result = evalAddOp( tn );
    else if ( testOp( tn, OpVal.subtractOp ) ) result = evalSubtractOp( tn );
    else if ( testOp( tn, OpVal.multiplyOp ) ) result = evalMultiplyOp( tn );
    else if ( testOp( tn, OpVal.unaryMinusOp ) ) result = evalUnaryMinusOp( tn );
    else if ( testOp( tn, OpVal.divideOp ) ) result = evalDivideOp( tn );
    else if ( testOp( tn, OpVal.modOp ) ) result = evalModOp( tn );  // chad added for %
    ...
    else if ( testOp( tn, OpVal.listOp ) ) result = evalListOp( tn );
  ...
  StickyNote evalExpression( Node tn )
  {
    ...
    else if ( testOp( tn, OpVal.colonOp ) ) result = evalColonOp( tn );
    else if ( testOp( tn, OpVal.listOp ) ) result = evalListOp( tn );
    else if ( testOp( tn, OpVal.aliasOp ) ) {
       String id =tn.left.info.toString();
  ...
  */
  StickyNote evalAddOp( Node tn )
  {
    ...
    //  System.out.println("===> Adding:" + s1);
          ...
          while (gl1.on()) gl1.move(); // move to the end of list g1
  ...
  StickyNote evalListOp( Node tn )
  {
    ...
    //l.reset();  l.insert(new MetaNode());
     if (debug) System.out.println ( "evalListOp returning " + l);
    // System.out.println ("Exiting ..."); System.exit(1);

     if (debug) System.out.println("Expression value:" + l  + "type: " + l.getClass());
     StickyNote stickynote = scope.alias( l );
     if (debug) System.out.println("Expression value:" + stickynote  + "type: " + stickynote.val.getClass());
     return stickynote;
  }//end evalListOp

  StickyNote evalSelect( Node tn )
  {
    if (debug) { System.out.println("Executing Select"); Parser.prettyPrint((TreeNode)tn);}
    Node cond, stmt;
    tn = tn.left();
    while ( tn != null ) // for each guardedStmt
    {
      //    System.err.println( "evalSelect:\t\tgetnfo = " + getnfo( tn ) );
  ...
  boolean evalCondition( Node tn )
  {
    ...
    boolean l,r;
     ...
     if ( testOp( tn, OpVal.trueOp ) ){ return true;}
     if ( testOp( tn, OpVal.otherwiseOp ) ){ return true;}
     if ( testOp( tn, OpVal.falseOp ) ){ return false;}
    ...
    if (   testOp(tn, OpVal.ltOp) 
       ...
       if      ( testOp( tn, OpVal.ltOp ) ) return ((GV)(lhs.val)).lt( rhs.val );
       else if ( testOp( tn, OpVal.leOp ) ) return ((GV)(lhs.val)).le( rhs.val );
       else if ( testOp( tn, OpVal.gtOp ) ) return ((GV)(lhs.val)).gt( rhs.val );
       else if ( testOp( tn, OpVal.geOp ) ) return ((GV)(lhs.val)).ge( rhs.val );
       ...
       else if ( testOp( tn, OpVal.neOp ) ) return ((GV)(lhs.val)).ne( rhs.val ); // ljm -- 6/20/04
    }
    else  // must be a function call, a variable or a literal constant
      
       if ( testOp( tn, OpVal.trueOp ) ) return (true);
       else if ( testOp( tn, OpVal.falseOp ) ) return (false);
       else  {
  ...
  {
     System.err.println("Emit not implemented");
     return null;
  }//end evalEmit
  ...
  StickyNote evalConcat( Node tn, boolean appendSpace )
  {
    ...
    while ( tn != null )
    {
      ...
      sn = evalExpression( tn );
        ...
        if (tn.right() != null && appendSpace) result = result.append(" ");
      tn = tn.right();
    }
  ...
  StickyNote evalFunctionDef( Node tn )
  {
    ...
    if (autotrace) {
       System.out.print ("Defining:" +name);
    }
  ...
  String buildMangledFunctionCallName(Node tn) {
    ...
    // System.err.println( "Got fname = "+ fname );
    return fname;
  ...
  StickyNote evalFunctionCall( Node tn )
  {
    ...
    if (debug) System.err.println( "fname = " + fname );
    // now, set up scope
    //
    ...
    //debug=true;
    if ( debug ) System.err.println( "Grabbing definition for  " + fname );
    ...
    if (debug) System.out.println("Got to here with fname="+fname);
    if (debug) System.out.println ("def is " + def.info + def.info.getClass());
    if ( def.info.val.toString().equals( "procedure") ) {  // built-in procedure
       if (debug) System.out.println("Calling builtin procedure " +fname);
       ...
       if (debug) System.out.println("Calling builtin function " +fname);
       if (fname.equals("*integer(1)")) {
          ...
          d = d - (int)d;
          if (d == 0.0) d = 0.0;   // to avoid the negative zero of Java
          returnVal = new StickyNote (new DoubleVal(d));
       }
...
tn);
          ...
          GenesisVal argument=evalExpression(call.right.left).val;
         if(argument instanceof NumberVal)  return(new StickyNote(true));
         else return(new StickyNote(false));
       }
          ...
          GenesisVal argument=evalExpression(call.right.left).val;
         if(argument instanceof TruthVal)  return(new StickyNote(true));
         else return(new StickyNote(false));
       }
       else if (fname.equals("*is(1)a_string?")){
         //if(!(call.right.left.info.val instanceof StringVal)) return(new StickyNote(false));
         GenesisVal argument=evalExpression(call.right.left).val;
         if(argument instanceof StringVal) return(new StickyNote(true));
         else return(new StickyNote(false));
       }
         ...
         GenesisVal argument=evalExpression(call.right.left).val;
         if(argument instanceof GenesisList) return(new StickyNote(true));
         else return(new StickyNote(false));
       } 
         ...
         if((argument instanceof Node) ) { return(new StickyNote(true));}
         else {return(new StickyNote(false));}
       }
         ...
         GenesisVal argument=evalExpression(call.right.left).val;
               if(argument instanceof NumberVal) return(new StickyNote("Number"));
         else if (argument instanceof StringVal) return(new StickyNote("String"));
         else if (argument instanceof Node)      return(new StickyNote("List")); 
         else if (argument instanceof GenesisList) return(new StickyNote("Iterator")); 
         else if (argument instanceof TruthVal) return(new StickyNote("TruthValue")); 
       } 
          ...
          */
          ...
          //debug = true;
          if (debug) System.out.println ("Processing iterator("  + call.right.left.info.val.toString() +")");
          if (call.right.left.info.val instanceof StringVal) {
             /*
          ...
          */
          ...
          if (call.right.left.info.val instanceof StringVal) {
             ...
             */
             ...
             printError("Iterator() is trying to form an iterator from non-iterator: " + sn
                        ,tn);
          }
          //System.out.println("EvalRefing" + call.right.left.info);
          GenesisVal argument = evalRef(call.right.left); 
          ...
          */
          ...
          // if it is a list, otherwise return 0
            ...
            if (argument instanceof Node) {
               ...
               do {
                  if (! (n instanceof MetaNode) )count++;
                  n = n.next();
               } while (n != null);
            }
    ...
    while ( call != null && def != null )
    {
        if (testType(call,OpVal.parameterOp)) {
           if (debug) System.err.println("Formal parameter is '" + def.left.info.val.toString() +"'");
           if (debug) System.err.println("Actual parameter is '" + evalExpression(call.left)+"'");
                ...
                if (testOp(formalParam,OpVal.aliasOp)) {
                   // letIdAliasSN (myScope, formalParam.left().info.val.toString(), evalExpression( actualParam ) );
                    ...
                    if (actualParam.info.val instanceof StringVal) {
                       ...
                       if (n == null) scope.setName(actualParam.info.val.toString(),new StickyNote());
                    }
    ...
    if (debug) System.out.println("Exiting evalFunctionCallOp"+returnVal);
    return returnVal;
...
//          System.out.println("Interpreting rangeNxN");
          ...
          if (testOp(pipeline.left.right, OpVal.rangeNxNxN_Op)) {
             ...
             if (incr <=0) printError("Increment in a range must be positive, not" + incr);
          }
...
*/
       ...
       //System.out.print("s " + iteratorName + " a GenesisList? ");
  ...
  {
       stmtCount++;
    if (debug) System.out.println("Entering evalUserPipe with " + tn);
    StickyNote returnVal = null;  // To store the returned value
    StickyNote result = null;
    if ( testOp( tn.left, OpVal.generatorCallOp ) )
    {
...
//debug = true;
       ...
       if (debug) System.out.println("1. Got to here with fname="+fname);
       if (debug) System.out.println ("def is " + def.info + def.info.getClass());
       ...
       // Now, we should have all the parameters linked.
       if (debug) System.out.println ("Parameters matched");
       Scope saveScope;
       ...
       while (itercond != null &&  !testOp( itercond, OpVal.whileOp )) {
          ...
          if (debug) System.out.println("Evaling in loop:"+itercond );
       }
   
       if (debug) System.out.println ("Parameters matched");
       // 1. Execute the generator's initialization
       if (debug) System.out.println ("Generator @first");
       def = (Node)(temp.val);
       Scope savedScope = scope;
       scope = myScope;
       evalStmt (genfirst);
       ...
       // Now begin the interpretation
       //Interpret each filter's initialization
       if (debug) System.out.println ("Task:init");
       Node guardedFilter = pipeline.right();
       Node filter;
       while (guardedFilter != null) {
          filter = guardedFilter.left.right;
       ...
       if (debug) System.out.println("Generator @last");
       savedScope = scope;  // save the scope
       scope = myScope;
       StickyNote cond = evalStmt(genlast);
       if (debug) System.out.println("@last truth value"+cond);
       ...
       if (debug) System.out.println("Generator condition = " + condition);
       ...
       if (debug) System.out.println("Iteration condition = " + condition);
   
       // 5.   if not, then run each task's iteration code and
                 //continue with step 3
       if (debug) System.out.println("Iterator @iter");
       ...
       while (condition) {
          ...
          evalStmt(geniter);
          scope = savedScope;
          if (debug) System.out.println("Evaluating condition");
...
*/
  ...
  StickyNote evalDelete( Node tn ) {
       stmtCount++;
     // debug = true;
     if (debug) {System.out.println("Calling Delete with");Parser.prettyPrint((TreeNode)tn); }
  ...
  */
  ...
  StickyNote evalAppend (Node tn) {
     ...
     // System.out.print("Appending " + expVal + " onto "); current.displayln();
     ...
     while (current.on()) current.move();  // move to end of list
  ...
  StickyNote evalPrepend (Node tn) {
     ...
     // System.out.print("Prepending " + expVal + " onto "); current.displayln();
     if (current.on())  { // list-exp is subscripted 
                          // or accessed through an iterator
        StickyNote sn = current.get();
        if (! (sn.val instanceof Node) 
