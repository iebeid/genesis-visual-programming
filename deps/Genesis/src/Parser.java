...
public class Parser {
    ...
    private static Hashtable <String,String> symtab = new Hashtable<String, String>();
...
public static void init() {
    ...
    // symtab.put("integer(), "builtin-function"); 
    symtab.put("integer", "function"); 
    symtab.put("integer()", "function"); 
    //symtab.put("concat", "function"); 
    //symtab.put("concat()", "function"); 
    ...
    symtab.put("iterator()","function");
    //symtab.put("copy", "function");   // to come
    //symtab.put("copy()", "function");   // to come
...
public void setSource(String fileName) {
    ...
    // System.out.println ("After setting source in Parser, lineNo="+t.lineNo());
    t.appendInput(fileName);
    // System.out.println ("After appending source in Parser, lineNo="+t.lineNo());
    t.processInput();
    // System.out.println ("After appending source in Parser, lineNo="+t.lineNo());
    // t.get();
    // System.out.println ("After calling get the first time in Parser, t="+t);
    /*  This code is to set an output destination for the parser.
     *  Since the parser shouldn't be printing normal output, we don't need
     *  to open an output file. Now errors...
     *
    try{
    //outfile = new File (filename + ".scm");
...
static String sinfo (TreeNode n) {
   String ans = "";
   //System.out.println ("sinfo label of n: " + n.getLabel());
   // if (n.getLabel() != null) ans = ans + n.getLabel() + ":";
...
static String  rootinfo (TreeNode n) {
   String ans = "";
   //System.out.println ("root label of n: " + n.getLabel());
   //if (n.getLabel() != null) ans = ans + n.getLabel() + ":";
   //System.out.println ("ans: " + n.getLabel());
   if (n.info.getVal() instanceof OpVal ) {
      //return tokenStr[((DoubleVal)n.info.getVal()).toInt()];

     OpVal o = (OpVal)(n.info.getVal());

     // edited by Wes 6/14/04
     if (debug) ans = ans + OpVal.opStr[ o.getVal() ] 
           + "(" + n.fileName() + ":"+ n.lineNo() + ":" + n.charPos() + ")";
     else 
     ...
     // edited by Wes 6/14/04
     if ( debug ) ans = ans + (n.info.toString())
           + "(" + n.fileName() + "::" + n.lineNo() + ":" + n.charPos() + ")";
     else {
        ans  = ans + (n.info.toString());
     }
...
static void traverse2(TreeNode tn) {
   if (tn != null) {
      ...
      System.err.println(rootinfo(tn));
      indent++;
...
static void traverse (TreeNode tn){
   //if (debug) System.err.println("Entering traverse");

   //System.err.println("Entering traverse with" + tn.context);
   if (tn != null) {
      for (int i = 0; i < indent; i++) System.err.print(' ');
      if (tn.left == null)  { // leaf node
         ...
         if ( tn.info.getVal() instanceof OpVal )  { // bizarre error omitting this brace and its matching
            ...
            if (ov.eq ( OpVal.stmtListOp) ) {
               ...
               System.err.println(sinfo(tn));
            }
         }
         else {
   //System.err.println("In traverse with" + tn.context);
            System.err.println(sinfo(tn));
         }
      }
      else{
      ...
      indent = indent + 3;
      TreeNode t = (TreeNode) tn.left;
      while (t != null) { traverse(t);t = (TreeNode) t.right; }
      indent = indent - 3;
   }
}
...
static void prettyPrint (TreeNode tn) {
  traverse(tn); 
  /*
   OpVal ov = (OpVal) tn.info.val; 
   
   if (ov.eq(OpVal.idNameOp)) {
      printIndent(); 
      System.out.println("Let " + tn.left + " " + tn.left.right);
        
   }
   else  if (ov.eq(OpVal.stmtListOp)) {
      TreeNode t = (TreeNode) tn.left;
      while (t != null) {
         prettyPrint(t);
         t = (TreeNode) t.right;
      ...
      OpVal.functionDefOp: 
        traverse(tn);
      OpVal.procedureDefOp: 
        traverse(tn);
      OpVal.returnStmtOp: 
        traverse(tn);
      OpVal.procedureCallOp: 
        traverse(tn);
      OpVal.pipeOp: 
        traverse(tn);
      OpVal.deleteOp: 
      ...
      OpVal.insertAfterOp: 
        traverse(tn);
      OpVal.insertBeforeOp: 
        traverse(tn);
      OpVal.stopStmtOp: 
        traverse(tn);
      OpVal.echoStmtOp: 
        traverse(tn);
      OpVal.generateStmtOp: 
        traverse(tn);
      OpVal.unaliasStmtOp:
        traverse(tn);
      OpVal.functionCallOp:
        traverse(tn);
      OpVal.generatorCallOp:
        traverse(tn);
      OpVal.nullStmtOp:
        traverse(tn);
*/
...
{
  int i;
  //  System.err.println ("Entering Parser::check with " + t.toString()); 
...
// check -- true iff the current token (t) is tokenType s
boolean check(int s)
{
  //  System.err.println ("Entering Parser::check with " + t.toString()); 
...
// consume -- replace the current token (t) with the next one
void consume () {
   t.get();
} 
...
void error(int s[]){
       ...
       // System.err.println("Error at line " +t );
       System.err.println("Error in file " + t.fileName() 
                          + " at line " + t.lineNo() + ", character " + t.charPos());
       System.err.println("Looking for any of the following: ");
       int i;
       for (i=0; i < s.length; i++)
           System.err.println("   " +tokenStr[s[i]]) ;
       System.err.println("\nFound: "+ tokenStr[t.tokenType]);
       System.err.println("*******************************************");
       System.err.println("Algorithm terminated!");
       System.err.println();
...
{
   Token    c,d;
      //System.err.println("Entering verify with " + t.toString());
...
void verify(int s)
{
   ...
   if (check(s))  t.get();
       ...
       System.err.println("Error in file " + t.fileName() 
                          + " at line " + t.lineNo() + ", character " + t.charPos());
       System.err.println("Looking for the following: ");
       int i;
       System.err.println("   " + tokenStr[s] ); //+ ":" + s);
       System.err.println("\nFound: "+ tokenStr[t.tokenType]);
...
**************************/
    if (debug) System.err.println("Entering parseProgram");
    TreeNode tn = parseStmtList();
    // edited by Wes Potts 6/14/04
    // the traverse() is not part of the if (debug) and I'm guessing
    // it should be
    if (debug) {System.err.println("Leaving parseProgram");traverse(tn);}
    if (check(eofSym)) {
       verify(eofSym);
    }
    else verify(firstOfStmt);
    return tn;
}
...
StmtList : Stmt
         | Stmt StmtList
Builds:  OpVal.stmtListOp
            Stmt 
            Stmt
            ...
***********************/
        if (debug) System.err.println("Entering parseStmtList");
    ...
    if (t != null && testOp(t,OpVal.printOp)) {
       // System.err.println("Running kluge check on:");
       // traverse(t);
       TreeNode temp = (TreeNode) t.left;
       ...
       if (temp.right() != null) {  // we have the kludge!
          t.right = (TreeNode) temp.right(); // link the proc call as a stmt
          temp.right = null; // remove the proc call from the print stmt
          t = (TreeNode) t.right; // move t to the proc stmt
       }
    }
    while (check(firstOfStmt)) {
       ...
       if (testOp(t,OpVal.printOp)) {
          ...
          while (temp.right() != null && !testOp(temp.right(), OpVal.procedureCallOp)) 
             temp = (TreeNode) temp.right;
          if (temp.right() != null) {  // we have the kludge!
             t.right = (TreeNode) temp.right(); // link the proc call as a stmtm
             temp.right = null; // remove the proc call from the print stmt
             t = (TreeNode) t.right; // move t to the proc stmt
          }
       }
    ...
    root.left = tn;
        if (debug) {System.err.println("Leaving parseStmtList");traverse(tn);}
    return root;
}
...
***********************/
TreeNode parseStmt() {
        if (debug) System.err.println("Entering parseStmt");
    TreeNode tn = null;   /// PERHAPS TreeNode(nullStmt)
    if      (check(letSym))   tn = parseLetStmt();
    //else if (check(emitSym))   tn = parseEmitStmt();
    else if (check(printSym))  tn = parsePrintStmt();
    else if (check(selectSym)) tn = parseSelectStmt();
    else if (check(insertSym)) tn = parseInsertStmt();
    else if (check(deleteSym)) tn = parseDeleteStmt();
    else if (check(appendSym)) tn = parseAppendStmt();
    else if (check(prependSym)) tn = parsePrependStmt();
    else if (check(callSym))   tn = parseProcedureOrGeneratorCall();
    else if (check(generateSym)) tn = parseIterationStmt();
    else if (check(returnSym)) tn = parseReturnStmt();
    else if (check(lbraceSym)) tn = parseBlock();
    else if (check(stopSym))   tn = parseStopStmt();
    else if (check(echoSym))   tn = parseEchoStmt();
    else if (check(unaliasSym)) tn = parseUnaliasStmt();
    else if (check(whileSym)) tn = parseWhileStmt();
    else if (check(untilSym)) tn = parseUntilStmt();
    ...
    else if (t.val != "")  // Somehow there is no more input ... treat as null stmt
        printError ("A statement cannot begin with "+ t.val);

    if (debug) {System.err.println("Leaving parseStmt");traverse(tn);}
    return tn;
}
...
**********************/
    ...
    if (debug) System.err.println("Entering parseBlock");
    verify (lbraceSym);
    TreeNode tn = parseStmtList();
    verify (rbraceSym);
    if (debug) {System.err.println("Leaving parseBlock");traverse(tn);}
    return tn;
}
...
**********************/
        ...
        if (debug) System.err.println("Entering parseTask");
    verify (lbraceSym);
    TreeNode tn = parseTask();
    verify (rbraceSym);
        if (debug) {System.err.println("Leaving parseTask");traverse(tn);}
    return tn;
}
...
int countReturns (Node tn) {
   //System.out.println ("In countReturns at " + tn);
   int count  = 0;
...
/*************************
Def :  Let <Id> Reference <Exp> ;
Builds  id <Reference>Op
           id
           Exp

Def :  Let <SubscriptedId> Reference <Exp> ;
Builds  id <Reference>Op
           SubscriptedId
           Exp

Def : Let <IdParameterList> Name <FunctionDef> ;
Builds  OpVal.functionDefOp
           IdParameterList
           FunctionDef


Def : Let <Id> Reference <ProcedureDef> ;
Builds  OpVal.procedureDefOp
           IdParameterList
           FunctionDef


//Def :  Let <Id> Name <GeneratorDef> ;
Builds  OpVal.generatorDefOp
           IdParameterList
           generatorDef
**************************/
TreeNode parseLetStmt(){
   if (debug) System.err.println("Entering parseLetStmt");
   ...
   if (check(reservedWordorId) || check(lparenSym)) { // maybe a proc or func or task
      if (t.val.equals("iterator")) {  //special case for iterator!
         ...
         if (check(lparenSym)) { // must be a proc/func/task
            foundParameter = true;
            TreeNode t = parseParmIdList();  // ljm -- 6/19/04
            if (tn == null) 
               tn = t;
            else{
               tn.right = t.left;               // ljm -- 6/19/04
               t.left = tn;                     // ljm -- 6/19/04
            }
            tn = t;
            if (debug) {
         ...
         else verify(new int[] {nameSym, aliasSym}); // ljm: 01/01/07
...
//System.out.println("Processing mangledName=" + mangledName);
         ...
         //System.out.println("returnCount is " + returnCount);
                  ...
                  //symtab.put(mangledName, "function"); 
                  if (debug) System.err.println("Entering: " + mangledName + " as " 
  ...
  //System.out.println("Proc:'" +mangledName + "' entered into symtab");
...
//               System.err.println("Entering: "+mangledName+ " as " + (String) symtab.get(mangledName));
            }
            else if (check(generatorSym)) {
               tn = (TreeNode) tn.left;
               TreeNode temp = new TreeNode(OpVal.generatorDefOp,tn, null);  
               symtab.put(mangledName, "generator"); 
   	    //System.out.println("Generator:'" +mangledName + "' entered into symtab");
     ...
     error(new int[] {idSym, lparenSym});
       ...
       if (debug) {System.err.println("Leaving parseLetStmt");
           TreeNode d = new TreeNode (OpVal.debugOp,tn,null);
          traverse(d);
       }
   return tn;
...
/***********************
EmitStmt : 'print' <Expression>
Builds   : OpVal.printOp
               Expression
***********************/
        if (debug) System.err.println("Entering parsePrintStmt");
    TreeNode tn = null;

    Context context = new Context(t);
    //System.out.println("Print context to " + context);
    verify (printSym);
    tn = parsePrintExpressionList();
    tn = new TreeNode (OpVal.printOp,tn,null);
        if (debug) {System.err.println("Leaving parsePrintStmt");traverse(tn);}
    tn.context = context;
    //System.out.println("Print context assigned to " + tn.context);
    //traverse(tn);
    return tn; 
}
...
***********************/
    ...
    tn = new TreeNode (OpVal.echoStmtOp,tn,null);
        if (debug) {System.err.println("Leaving parseEchoStmt");traverse(tn);}
    return tn; 
}

TreeNode parseProcedureOrGeneratorCall(){
/**************************
ProcedureCall :[Call] Id '(' ExpressionList ')' [WHILE|UNTIL]
Builds : OpVal.procedureCallOp
            Id
            ExpressionList
**************************/
    if (debug) System.err.println("Entering parseProcedureOrGeneratorCall");
    if ( check(callSym) ) verify (callSym); // toss away ... just punctuation
    ...
    if (testOp(tn,OpVal.generatorCallOp)) {
       ...
       if (debug) {System.err.println("Leaving parseIterationStmt");traverse(tn);}
    if (debug) {System.err.println("Leaving parseProcedureCall");traverse(tn);}
    if (debug) { System.err.println("ppc: "); traverse(tn);}
    return tn; 
}


/*************************
PipeStmt :  GenerateStmt
            PIPE
            Pipeline
Builds:  OpVal.pipeOp
           GenerateStmt
...
TreeNode parseIterationStmt() {
        if (debug) System.err.println("Entering parseIterationStmt");
...
TreeNode parsePipeline() {
        if (debug) System.err.println("Entering parsePipeline");
    TreeNode tn; 
    tn = parseGuardedTask();
    if (check(pipeSym)) {
       verify(pipeSym);
       tn.right = parsePipeline();
        ...
        if (debug) {System.err.println("Leaving parsePipeline");traverse(tn);}
    return tn;
}
...
Builds: OpVal.guardedTaskOp 
           OpVal.condOp
           OpVal.taskOp 
***************************/
TreeNode parseGuardedTask() {
        if (debug) System.err.println("Entering parseGuardedTask");
    TreeNode tn;
    
    if (check(whenSym)) { verify(whenSym);
    //if (check(lparenSym)) { // complex case
        //verify(lparenSym);
    ...
    tn = new TreeNode (OpVal.guardedTaskOp,tn, null);
        if (debug) {System.err.println("Leaving parseGuardedTask");traverse(tn);}
    return tn;
}
/***************************
Task : ATBEGIN ':'  Stmt ATEACH ':'  Stmt ATEND ':' Stmt
       | ATBEGIN ':'  Stmt ATEACH ':'  Stmt
       | ATBEGIN ':'  Stmt ATEND ':' Stmt
       | ATBEGIN ':'  Stmt
       | ATEACH ':'  Stmt ATEND ':' Stmt
       | ATEACH ':'  Stmt
       | ATEND ':' Stmt
       | Stmt

Builds: OpVal.taskOp 
           Stmt  (null if omitted)
           Stmt  (null if omitted)
           Stmt  (null if omitted)
...
TreeNode parseTask() {
    ...
    if (check(new int[] {atbeginSym,ateachSym,atendSym})) { // complex case
       if (check(atbeginSym)) {
          verify(atbeginSym); verify(colonSym);
          tn = parseStmt();
       }
       else 
       ...
       if (check(atendSym)) {
          verify(atendSym); verify(colonSym);
...
Builds: OpVal.whileOp
           Condition
**********************/
TreeNode parseWhile() {
        if (debug) System.err.println("Entering parseWhile");
    TreeNode tn;
    verify (whileSym);
    tn = parseExpression(true);
    tn = new TreeNode (OpVal.whileOp, tn, null);
        if (debug) {System.err.println("Leaving parseWhile");traverse(tn);}
    return tn;
}
...
TreeNode parseUntil() {
        if (debug) System.err.println("Entering parseUntil");
    TreeNode tn;
    verify (untilSym);
    tn = parseExpression(true);
    tn = new TreeNode (OpVal.notOp, tn, null); 
    tn = new TreeNode (OpVal.whileOp, tn, null);
        if (debug) {System.err.println("Leaving parseUntil");traverse(tn);}
    return tn;
}
...
TreeNode parseSource(){
        if (debug) System.err.println("Entering parseSource");
    TreeNode tn;
    if (check (new int[]{ltSym,
       ...
       verify(idSym);
        ...
        if (debug) {System.err.println("Leaving parseSource");traverse(tn);}
    return tn;
}
...
Builds: OpVal.returnOp
           Expression
***********************/
        if (debug) System.err.println("Entering parseReturnStmt");
    TreeNode tn = null;

    verify (returnSym);
    tn = parseExpression(true);
    tn = new TreeNode (OpVal.returnStmtOp,tn,null);
        if (debug) {System.err.println("Leaving parseReturnStmt");traverse(tn);}
    return tn; 
}
...
Builds: OpVal.stopOp
***********************/
        if (debug) System.err.println("Entering parseStopStmt");
    TreeNode tn = null;
    Context context = new Context(t);
    // System.out.println("Setting contxt for stop to (@" + t.lineNo() + "/" + t.charPos() +")");
    // System.out.println("Token is "+ t);
    verify (stopSym);
    tn = new TreeNode (OpVal.stopStmtOp,tn,null);
    tn.context = context;
        if (debug) {System.err.println("Leaving parseStopStmt");traverse(tn);}
    return tn; 
}
...
Builds: SelectBody  (transparently)
           
************************/
//debug=true;
        if (debug) System.err.println("Entering parseSelectStmt()");

    verify (selectSym);
    TreeNode tn = parseSelectBody();
        if (debug) {System.err.println("Leaving parseSelectStmt()");traverse(tn);}
    return tn;
}    
...
TreeNode parseSelectBody() {
        if (debug) System.err.println("Entering parseSelectBody()");
    TreeNode tn  = null;
    if (check (lbraceSym)) {
       consume(); 
       tn=parseSelectList();
    ...
    tn = new TreeNode (OpVal.selectOp,tn,null);
        if (debug) {System.err.println("Leaving parseSelectBody()");traverse(tn);}
    return tn; 
}

/**********************
SelectList : GuardedStmt 
           | GuardedStmt SelectList
Builds: GuardedStmt
        GuardedStmt
        GuardedStmt
        ...
...
TreeNode parseSelectList(){
        if (debug) System.err.println("Entering parseSelectList");
    ...
    if (check (new int []{
       ...
       tn = new TreeNode (OpVal.guardedStmtOp,tn,null);
        if (debug) {System.err.println("Leaving parseGuardedStmt");traverse(tn);}
    }
    else {
	    //System.out.println("Sym  =" + t + "  "+ check(trueSym));
       /* 
    ...
    // debug=true;
    if (debug) {System.err.println("Leaving parseSelectList");traverse(tn);}
    return tn;
}

TreeNode parseGuardedStmt() {
        if (debug) System.err.println("Entering parseGuardedStmt");
...
TreeNode parseFile(){
        if (debug) System.err.println("Entering parseFile");
    TreeNode tn;
    verify (fileSym);
    tn = new TreeNode(t);
    verify(new int[]  {idSym,stringSym});
    tn = new TreeNode(OpVal.fileOp,tn,null);
        if (debug) {System.err.println("Leaving parseFile");traverse(tn);}
    return tn; 
}
...
*/
TreeNode parseRange(){
        if (debug) System.err.println("Entering parseRange");
    TreeNode tn;
	...
	// else ... it is simply an expression ... implying a list
        ...
        return tn = null;
        if (debug) {System.err.println("Leaving parseRange"); traverse(tn);}
    return tn;
}
...
*****************************/
    //    debug=true;
        if (debug) {System.err.println ("Entering parseExpressionList"); }
    ...
    if (testOp(tn,OpVal.procedureCallOp)) { // proc call included in print expr list
       ...
       tn.right = parsePrintExpressionList();
        ...
        if (debug) {System.err.println ("Leaving parseExpressionList");traverse(tn);}
    return tn;
}
...
/*************************
Exp : CondExp or CondTerm
    | CondTerm
Builds: OpVal.orOp
          OpVal.condExpOp
          OpVal.condTermOp
*************************/
        if (debug) System.err.println ("Entering parseExpression");
    ...
    if (debug) {System.err.println("Leaving Expression OpVal.Op with " + tn.info);traverse(tn);}
...
/*************************
CondTerm : CondTerm and CondPrimary
     | CondPrimary
...
Builds: OpVal.andOp
           ...
           OpVal.condprimaryOp
*************************/
        if (debug) System.err.println ("Entering parseCondTerm");
    ...
    if (debug) {System.err.println("Leaving CondTerm OpVal.Op with " + tn.info);traverse(tn);}
    return tn;
}
...
CondPrimary:  not ConditionPrimary
Builds: OpVal.notOp
          OpVal.ConditionPrimaryOp
*************************/
        if (debug) System.err.println("Entering parseCondPrimary");
    TreeNode tn = null;
    ...
    if (debug) {System.err.println("Leaving CondPrimary OpVal.Op with " + tn.info);traverse(tn);}
    return tn;
}
...
Condition : Expression operator Expression 
          | Expression
***************************/
        if (debug) System.err.println ("Entering parseCondition");
    TreeNode tn = null;
    Context context = new Context(t);
...
if (check(trueSym)) {
...
else  */
    {
       ...
       //TreeNode tn = new TreeNode (t.tokenType, e, null);
       if  (!(Token.commaBreak || Token.lineBreak) 
          ...
          tn =e;          
           ...
           if (debug) {System.err.println ("Leaving parseCondition");traverse(tn);}
    }
    return tn;
}
...
Exp : Exp + Term
    ...
    | Term
*************************/
        if (debug) System.err.println ("Entering parseArithmeticExpression");
    TreeNode tn =parseTerm(withFunction);   
    while (check(new int [] {plusSym,minusSym}) 
           && ! (Token.commaBreak)){
        if (debug) System.err.println ("Entering parseArithmeticExpression Loop");
       if (check(plusSym)){
          verify(plusSym);
    ...
    if (debug) {System.err.println("Leaving ArithmeticExpression OpVal.Op with " + tn.info);traverse(tn);}
    return tn;
}
...
Builds: OpVal.divideOp 
           OpVal.primaryOp             
           OpVal.primaryOp             
    or
Builds: OpVal.multiplyOp 
           OpVal.primaryOp             
           OpVal.primaryOp
    or
Builds: OpVal.modOp          
           ...
           OpVal.primaryOp
*************************/
        if (debug) System.err.println ("Entering parseTerm");
    ...
    if (debug) {System.err.println("Leaving parseTerm OpVal.Op with " + tn.info);traverse(tn);}
    return tn;
}

TreeNode parsePrimaryOrFunction (boolean withFunction) {
        if (debug) System.err.println ("Entering parsePrimaryOrFunction");
   String name = "";
   TreeNode tn;
...
TreeNode parsePossibleFunctionCall (boolean withFunction, String name) {
    //debug = true;
        if (debug) System.err.println ("Entering parsePossibleFunctionCall with" + name);
   ...
   if (debug) System.err.println("Here1: "+ t + "/" + newname + "/" + partialname);
   Object obj = symtab.get(newname);
   if (debug) System.err.println("Here2: "+ t + "/" + obj + "/" + partialname);
   if (debug) System.out.println("Checking for reserved word or id" + t);
   while (check(reservedWordorId) && obj != null) { // newname is a valid extension
      name = newname;
      possibleFunctionCall = true;
      verify(reservedWordorId);
      if (debug) System.err.println("Here3:" + t + "/" + newname + "/" + partialname);
      if (check (reservedWordorId)) { // see if this id can be appended
         ...
         if (debug) System.err.println("Here4:" + t + "/" + newname + "/" + partialname);
         //verify(reservedWordorId);
         if (debug) System.err.println("Here5:" + t + "/" + obj + "/" + name);
         if (obj == null) { // revert back to previous name
            ...
            while (check(reservedWordorId) && obj != null) {
               ...
               verify(reservedWordorId);
               newname = name + "_" + t.val;
               if (debug) System.err.println("Here6"+ t + "/" + newname + "/" + partialname);
               obj = symtab.get(newname);
               if (obj != null)
               partialname = partialname +  "_" +t.val;
            }
         }
       if (debug) System.err.println("Here7:"+  t + "/" +newname + "/" + partialname);
      ...
      newname = name +"()";
      if (debug) System.err.println("Here8:" + t + "/" + newname + "/" + partialname);
      obj = symtab.get(newname);
      if (debug) System.err.println("Here9:" + t + "/" + obj + "/" + partialname);
      if (debug) System.err.println("Here9.5:" + t + "/" + obj + "/" + partialname);
      ...
      if ( obj != null ) {
         if (check(lparenSym)) {
            ...
            // System.err.println("Here4:" + t.val );
            // traverse(n);
            n = (TreeNode) n.right;
            Node t = n.left; int count = 0;
            verify (rparenSym);
             if (debug) System.err.println("Here10:" + n );
         }
         else while (check (numberSym) || check(stringSym)){
           
         boolean firsttime = true;
         if (check (numberSym)){
            if (debug) System.err.println ("Extending routine name "+newname + " with " + t);
            name = newname;
            realFunctionCall = true;  // if it looks like a duck ...
  ...
  //debug=true; 
      if (debug) System.err.println("NEWNAME is " + newname);
      if (debug) System.err.println("NAME is " + name);
      if (debug) System.err.println("possible function call is " + possibleFunctionCall);
      if (debug) System.err.println("real function call is " + realFunctionCall);
   if (realFunctionCall) {
      if (debug) System.err.println("Real function call: NAME is " + name);
      obj = symtab.get(name);
      if (debug) System.err.println("Here11: obj= " +(String)obj ) ;
      if (obj == null || (String)obj == "partial") {
         printError("Error: " + name + " is not a valid function call");
      }
      if ((String)obj == "function") {
         tn = new TreeNode(OpVal.functionCallOp, (TreeNode)tn.right() ,null);
         if (debug) System.err.println("Here12: tn=");
         if (debug) traverse(tn);
      }
      else if ((String)obj == "procedure") {
         tn = new TreeNode(OpVal.procedureCallOp,(TreeNode)tn.right() ,null);
      }
      else if ((String)obj == "generator") {
      ...
      if (debug) System.err.println("Lonely variable: " + name + "/" + t.val + "/" + possibleFunctionCall);
      //tn = new TreeNode (t.val); verify(idSym);
   ...
   if (debug) System.err.println("Returning " + tn.info + "from parsePossibleFunctionCall");
   if (debug) traverse2(tn);
   //debug = false;
   return tn;
}
...
Primary : idSym '(' ExpressionList ')'
Builds: OpVal.functionCallOp
           idSym
           OpVal.expressionListOp
*************************/
    //  debug=true;
        if (debug) System.err.println("Entering parsePrimary");
    TreeNode tn = null;
...
*/
    ...
    if (debug) {System.err.println("Leaving Primary OpVal.Op with " + tn.info);traverse(tn);}
    // if we didn't get an ID or a '('  or ...
...
TreeNode parseList(int matchtype) {
        if (debug) System.err.println("Entering parseList looking for "+tokenStr[matchtype]);
    verify (new int[]{ltSym, lbracketSym});
    TreeNode tn;
    TreeNode t = parseListSeq(matchtype);
    if (t == null) tn = new TreeNode(OpVal.listOp);
    else{
      tn = new TreeNode (OpVal.listOp, t, null);
    }

    verify (new int[]{gtSym, rbracketSym});
    
        if (debug) {System.err.println("Leaving parseList");traverse(tn);}
    return tn;
}
...
Builds: 
          Atom or List or alias Atom 
          Atom or List or alias Atom
          Atom or List or alias Atom
          ...
***************************/
TreeNode parseListSeq (int matchtype){
        if (debug) System.err.println("Entering parseListSeq");
    TreeNode tn = null;
    if (check (matchtype)) 
       return null;
    if (check (ltSym))
...
/*
        if (check(labelSym)){                                                    //added for records chad 
          ...
          temp = new Token(t);           // copy the current Token t
...
*/
        // Don't change this to parseExpression or there are big problems !!!
          tn = parsePrimary();
    ...
    tn.right = parseListSeq(matchtype);
        if (debug) {System.err.println("Leaving parseListSeq");traverse(tn);}
    return tn;
}

TreeNode parseGenerateStmt(){
/*************************
GenerateStmt : 'generate' GeneratorRef 'from' Source
             | 'generate' GeneratorRef 'from' Source While
             | 'generate' GeneratorRef While
             | 'generate' GeneratorRef Until
Builds OpVal.generateOp
         GeneratorRef
         Source
         While  (not present ... if not present!)
*************************/

    if (debug) System.err.println("Entering parseGenerateStmt");
    verify (generateSym);
    ...
    tn = new TreeNode (OpVal.generateStmtOp,tn,null);
        if (debug) {System.err.println("Leaving parseGenerateStmt");traverse(tn);}
    return tn; 
}
...
Builds : OpVal.generatorRefOp
            Id
            ParameterList
****************************/

        if (debug) System.err.println("Entering parseGeneratorRef");
    ...
    tn = new TreeNode (OpVal.generatorRefOp,tn,null);
        if (debug) {System.err.println("Leaving parseGeneratorRef");traverse(tn);}
    return tn; 
}
...
/*************************** 
//ParameterList : ID
              //| ID ',' ParameterList   ljm -- 6/19/04
              //| ID  ParameterList   ljm -- 6/19/04
              //| null
    Each ID may be optionally preceded by 'alias' or 'iter'
...
***************************/
        ...
        if (debug) System.err.println ("Entering parseParameterList");
    TreeNode tn = null;
    if (check (idSym)) {
       tn = new TreeNode (t.val);
       tn.setContext(t);
    ...
    if (debug) {System.err.println ("Leaving parseParameterList");traverse(tn);}
    return tn;
}
...
TreeNode parseIdParmList() {
        if (debug) System.err.println ("Entering parseIdParmList");
    TreeNode tn = null;
    tn = parseIdSeq();
    if (check (lparenSym))
       tn.right = parsePIList();
       if (debug) {System.err.println ("Leaving parseIdParmList");traverse(tn);}
    return tn;
}
...
TreeNode parseIPList() {
        if (debug) System.err.println ("Entering parseIPList");
    ...
    if (check (lparenSym))
       ...
       // if (check(idSym)) {
        ...
        if (debug) {System.err.println ("Leaving parseIPList");
           TreeNode d = new TreeNode (OpVal.debugOp,tn,null);
           traverse(d);
        }
...
TreeNode parseParmIdList() {
   // debug=true;
        if (debug) System.err.println ("Entering parseParmIdList");
    TreeNode tn = null;
    tn = parseParameter();
    //if (check (idSym))
    if (check (reservedWordorId))
       tn.right = parseIPList();
        if (debug) {System.err.println ("Leaving parseParmIdList");
           TreeNode d = new TreeNode (OpVal.debugOp,tn,null);
           traverse(d);
        }
...
TreeNode parsePIList() {
        if (debug) System.err.println ("Entering parsePIList");
    ...
    //if (check (idSym))
       ...
       //if (check(idSym)) {
        ...
        if (debug) {System.err.println ("Leaving parsePIList");traverse(tn);}
    return tn;
}
...
String parseIdString() {
        if (debug) System.err.println ("Entering parseIdString"); 
    ...
    //if (check(idSym))
       ans = ans + "_" +  parseIdString();
        if (debug) {System.err.println ("Leaving parseIdString with "); 
...
TreeNode parseIdSeq(){
        if (debug) System.err.println ("Entering parseIdSeq");
    String idString = parseIdString();
    TreeNode tn = new TreeNode (idString);
    tn.setContext(t);
        if (debug) {System.err.println ("Leaving parseIdSeq"); traverse(tn);}
    return tn;
}
...
TreeNode parseParameter(){
        if (debug) System.err.println ("Entering parseParameter");
    verify(lparenSym);
    ...
    tn = new TreeNode (OpVal.parameterOp,tn,null);
        if (debug) {System.err.println ("Leaving parseParameter");traverse(tn);}
    return tn;
}
...
String mangle (TreeNode tn) {
   //debug=true;
   if (debug)  System.err.println("mangling:" + tn);
   Node n = tn;
   if ( n != null ) n = n.left();
   int i;
      ...
      if ( testType( n, new StringVal("") ) ) {
         ...
         for (i = 0; i < s.length(); i++) {
             if (s.charAt(i) == '_') {
                ...
                // System.err.println( "Found subprog def:" + 
...
*************************/
        if (debug) System.err.println("Entering parseProcedureDef");
    verify(procedureSym);
    TreeNode tn = parseBlock();
        if (debug) {System.err.println("Leaving parseProcedureDef");traverse(tn);}
...
*************************/
        //debug=true;
        if (debug) System.err.println("Entering parseTaskDef");
    TreeNode tn = null ;
    if (check (taskSym)) {
       verify(taskSym);
...
//debug=true;
        if (debug) {System.err.println("Leaving parseTaskDef");traverse(tn);}
//debug=false;
//System.exit(1);    
    return tn;
}
...
/********************
InsertStmt : 'insert' Expression-1 'before' Expression-2 
InsertStmt : 'insert' Expression-1 'after'  Expression-2

NB: Expression-2 must resolve to a list position
    Expression-3 must resolve to a list 

Builds:
    insertBeforeOp (or insertAfterOp)
       Expression-1
       Expression-2
       Expression-3
...
TreeNode parseInsertStmt(){
   ...
   /*
   if (check (labelSym)) {
      Token temp = new Token(t);     // copy the current Token t
      verify(labelSym);              // gets next token
   ...
   */
   ...
   tn.left = e1; e1.right = e2; // e2.right = e3;
   return tn;
}
...
/***********************
UnaliasStmt : 'unalias' id
Builds: OpVal.unaliasStmtOp
***********************/
        if (debug) System.err.println("Entering parseUnaliasStmt");
    Context context = new Context(t);
    // System.out.println("Setting contxt for stop to (@" + t.lineNo() + "/" + t.charPos() +")");
    // System.out.println("Token is "+ t);
    verify (unaliasSym);
    ...
    tn.context = context;
        if (debug) {System.err.println("Leaving parseUnaliasStmt");traverse(tn);}
    return tn; 
}
...
public static void main(String args[]) 
 {
   ...
   if (args.length != 1 && args.length != 2) {
      System.err.println("Usage: java Parser filename.gen");
      System.exit(1);
   }
   ...
   if (args.length == 2) {
      ...
      if ( args[0].equals("debug")) {
         ...
         System.err.println("Parser: Invalid option flag of " + args[0] + " ignored");
      }
   }

   sourcefile = args[filevar];
   
   //GenesisIO io = new GenesisIO();
   //io.setInputFile( args[filevar] );
   
   Parser p = new Parser(sourcefile);
   p.setSource(sourcefile);
   System.out.println("Parsing:");
   System.out.println(Token.sourcePgm.toString(true));
   if (debug) System.err.println("Start parsing\n"); 
   TreeNode tn = p.parse(); 
   if (debug) System.err.println("Done parsing\n"); 
   if (debug) { traverse2(tn);
      System.out.println("--------------------");
      traverse(tn);
      System.out.println("--------------------");
   ...
   //p.out.println ("(define GenesisPgm \'((newline)(newline)");

   //p.out.println("))");
   //p.out.println("(interp genesispgm)");
   //p.out.close();
   traverse(tn);
   System.out.println(Token.sourcePgm.toString(true));
