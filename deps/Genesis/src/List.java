...
boolean atLast() {
    ...
    if (p.point == null && p.head.right() == null) return true;
    if (p.point != null && p.point.right() != null && p.point.right().right() == null) return true;
...
public void display () { System.out.print(this);}
public void displayln () { System.out.println(this);}
// ------------ Test procedures ----------- 
void ListsTest()
{
...
//  with (A.P)^)
  if (p.head == null){
     VR = (p.point == null);
     if (! VR) System.out.println("Violate 1");
  }
  else if (p.point == null){
     VR =  noLoops();
     if (! VR)System.out.println("Violate 2");
  }
  else {
     VR =  noLoops();
     if (! VR) System.out.println("Violate 3");
  }
  return VR;
}
...
public static void main (String a[]){
   ...
   System.out.println("atLast is true"); 
   xl.displayln();
  ie.displayln();
   for ( int i = 0; i < 10; i++ ) {
     ie = new StickyNote(i);
   ...
   l.left();
   l.displayln();
   System.out.println("Starting backward through the list");
   while (! l.atFirst() ) {
     StickyNote e = l.get();  
     e.displayln(); 
     l.displayln();
...
/*
Node copyNodes(List SL, StickyNote DL, Node SN)
// SN is a list node from the source list SL
// DN is the node in the destination list DL
// A  is passed in order to set the p.point
{ StickyNote v;
   Utility.entering("copyNodes");
   if (SN == null){
      Utility.leaving("copyNodes..no copy");
      return null;
   }
   else {
      Node DN = new Node();
      DN.right() = copyNodes(SL, DL, SN.right());
      if (SL.p.point == SN) // found the p.point
          DL.p.point = DN;   // set it in the destination list
      Utility.leaving("copyNodes node copied");
      return DN;
   }
