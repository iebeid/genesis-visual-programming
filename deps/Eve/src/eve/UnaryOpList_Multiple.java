 ...
 * @author Larry Morell
 */
public class UnaryOpList_Multiple extends EveInstruction
	...
	public StickyNote eval(EnvironmentVal env,Instruction theInstruction) {
		...
		if (!(sn.getVal() instanceof DoubleVal)) {
			System.err.println("Attempting to negate a non-number:" + sn);
			System.exit(-1);
		}
		if (negative) {
			double d = ((DoubleVal)(sn.getVal())).getVal();
			sn = new StickyNote(-d);
		}
		return sn;
	}
