...
public class FunctionDefinition extends EveInstruction {

	private static String rule = "<FunctionDefinition> ::= function ( <FormalParameterList> ) { <ExpressionList> } ";
   ...
   private InstructionList formalParameters;
	private InstructionList body;
	@Override
	public Instruction createInstruction(InstructionList il) {
		FunctionDefinition fd = new  FunctionDefinition();
		fd.formalParameters = ((EveInstruction) il.get(2)).getInstructionList();  // <FormalParameterList>
      fd.body = ((EveInstruction) il.get(5)).getInstructionList();  // <Expression>
		return  fd;
	}

   public CodeVal getFormalParameters () { return new CodeVal(formalParameters);}
	public CodeVal getBody() { return new CodeVal(body);}
	@Override
	...
	/**
	 * Wrap the current function definition as a CodeVal and return it
	 * @param env
	 * @return
	 */
	@Override
	public StickyNote eval(EnvironmentVal env) {
		CodeVal code = new CodeVal(this);
		return new StickyNote (code);
	}
