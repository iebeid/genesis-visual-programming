...
public class ConditionTerm_And extends GenesisInstruction
                                     {

	private static String rule = "<ConditionTerm> ::= <ConditionPrimary> <AndOp> <ConditionTerm>";
	...
	public static void initialize() {  // static initialization
		GRAMMAR.addRule(syntaxRule,
				  new ConditionTerm_And(),
				  new GenericParser(syntaxRule.lhs()));
	}
	InstructionList il;
   public ConditionTerm_And(){ }

	@Override
	public Instruction createInstruction(InstructionList il) {
	   ConditionTerm_And exp = new ConditionTerm_And();
		...
		if (third instanceof ConditionTerm_And) {
			ConditionTerm_And term = (ConditionTerm_And) third;
